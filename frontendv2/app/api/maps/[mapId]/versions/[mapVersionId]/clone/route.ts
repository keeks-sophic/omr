...
import type { CloneMapRequest } from "@/lib/api/types";
...
export async function POST(req: Request, ctx: { params: Promise<{ mapId: string; mapVersionId: string }> }) {
  ...
  let body: CloneMapRequest;
  try {
    body = (await req.json()) as CloneMapRequest;
  } catch {
    body = {};
  }

  const res = await backendFetch(ApiRoutes.maps.clone(mapId, mapVersionId), {
    method: "POST",
