...
import type { CreatePointRequest } from "@/lib/api/types";
...
export async function POST(req: Request, ctx: { params: Promise<{ mapId: string; mapVersionId: string }> }) {
  ...
  try {
    body = (await req.json()) as CreatePointRequest;
  } catch {
    return NextResponse.json({ error: "invalid_json" }, { status: 400 });
  }

  const res = await backendFetch(ApiRoutes.maps.points(mapId, mapVersionId), {
    method: "POST",
