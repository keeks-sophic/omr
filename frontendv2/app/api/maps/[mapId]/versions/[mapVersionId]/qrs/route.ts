...
import type { CreateQrRequest } from "@/lib/api/types";
...
export async function POST(req: Request, ctx: { params: Promise<{ mapId: string; mapVersionId: string }> }) {
  ...
  let body: CreateQrRequest;
  try {
    body = (await req.json()) as CreateQrRequest;
  } catch {
    return NextResponse.json({ error: "invalid_json" }, { status: 400 });
  }

  const res = await backendFetch(ApiRoutes.maps.qrs(mapId, mapVersionId), {
    method: "POST",
