        ...
        try
        {
            _cmdSub = _nats.Subscribe(opts.CommandSubject, (s, e) =>
            {
                ...
                try
                {
                    ...
                    using var scope = _scopeFactory.CreateScope();
                    var planner = scope.ServiceProvider.GetRequiredService<IRoutePlannerService>();
                    var mapsvc = scope.ServiceProvider.GetRequiredService<IMapService>();
                    var plan = planner.Plan(new backend.DTOs.RouteRequestDto
                    {
                        RobotIp = ip!,
                        MapId = mapId,
                        DestX = dx,
                    ...
                    if (plan is null)
                    {
                        _logger.LogWarning("Route planning failed for {Ip} dest=({X},{Y}) mapId={MapId}", ip, dx, dy, mapId);
                        return;
                    }
                    var graph = mapsvc.GetGraph(plan.MapId);
                    var coords = graph?.Nodes
                        .Where(n => plan.NodeIds.Contains(n.Id))
                        .Select(n => new { id = n.Id, x = n.X, y = n.Y })
                        .ToArray() ?? Array.Empty<object>();
                    var payload = new
                    {
                        ip = ip,
                        command = "route.assign",
                        route = new { robotIp = ip, mapId = plan.MapId, nodeIds = plan.NodeIds, pathIds = plan.PathIds },
                        nodes = coords,
                        speed = 0.1,
                        ts = DateTime.UtcNow
                    };
                    _nats.PublishJsonAsync(_options.Value.CommandSubject!, payload).ConfigureAwait(false);
                    _logger.LogInformation("Dispatched route.assign to {Ip} nodes={Count}", ip, plan.NodeIds.Length);
                }
                catch (Exception ex)
                {
