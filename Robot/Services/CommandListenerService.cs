...
public class CommandListenerService
{
    ...
    private NATS.Client.IAsyncSubscription? _sub;
    ...
    public Task StartAsync(string name, string ip, string subject, CancellationToken token)
    {
        if (_sub != null) return Task.CompletedTask;
        try
        {
            _sub = _nats.Subscribe(subject, (s, e) =>
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(targetIp) && string.Equals(targetIp, ip, StringComparison.OrdinalIgnoreCase) && !string.IsNullOrWhiteSpace(command))
                    {
                        if (string.Equals(command, "route.assign", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            // optional enriched nodes with coordinates and speed
